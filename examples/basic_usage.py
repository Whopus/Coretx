...
from locagent_kernel import CodeLocator, LocAgentConfig, quick_localize, create_locator
...
def basic_localization_example():
    ...
    # Method 1b: Quick localization with custom OpenAI configuration
    print("\n=== Quick Localization with Custom OpenAI Config ===")
    try:
        # Example with custom API key and base URL
        results = quick_localize(
            repo_path=repo_path,
            problem_description=problem,
            model_name="gpt-4",
        ...
        print("Results with custom OpenAI config:")
        print(f"  Found {len(results['localization_results'].get('files', []))} files")
        print(f"  Found {len(results['localization_results'].get('classes', []))} classes")
        print(f"  Found {len(results['localization_results'].get('functions', []))} functions")
            
    except Exception as e:
        print(f"Custom OpenAI config localization failed: {e}")
    ...
    config.agent.api_key = "your-api-key-here"  # Set OpenAI API key
    config.agent.api_base = "https://api.openai.com/v1"  # Set OpenAI base URL
    ...
    # Method 3: Using create_locator convenience function
    print("\n=== Using create_locator with OpenAI Config ===")
    try:
        # Create locator with OpenAI configuration
        ...
        print("create_locator Results:")
        print(f"  Iterations: {results['iterations']}")
        print(f"  Total tokens: {results['total_tokens']}")
        print(f"  Files found: {len(results['localization_results'].get('files', []))}")
        
    except Exception as e:
        print(f"create_locator failed: {e}")
...
def configuration_example():
    ...
    custom_config.agent.api_key = "your-api-key-here"
    custom_config.agent.api_base = "https://api.openai.com/v1"
